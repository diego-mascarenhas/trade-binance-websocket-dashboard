...
def run_tp_manager(args: argparse.Namespace, is_long: bool | None) -> None:
    ...
    if not args.execute:
        ...
        print(f"\n{DIM}DRY-RUN — add --execute to auto-manage (place/replace) the TP.{RESET}")
        return
...
def main() -> None:
    ...
    if args.supervise:
        if not args.execute:
            print(f"{DIM}--supervise needs --execute (it places/re-arms real orders).{RESET}")
            return
        supervise_loop(args)
        return
    ...
    if not args.execute:
        print(f"\n{DIM}DRY-RUN — no orders sent. Re-run with --execute to place the grid "
